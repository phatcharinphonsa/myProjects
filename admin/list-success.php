...
<body>
    <section class="home-section mt-5">
        ...
        <h5 class="mt-2 mb-5 text-center">รายการสั่งซื้อสินค้าที่จ่ายชำระแล้ว</h5>
        <div>
            <table class="table" style="width: 100%;">
                ...
                <tbody>
                    ...
                    $i = 1;
                    $total = 0;
                    $sql = "SELECT * FROM `order` JOIN `agents` ON `order`.`agent_id` = `agents`.`agent_id` WHERE `order_status` = 2 ORDER BY reg_date DESC";
                    ...
                    <tr>
                        ...
                        $mpdf->Output('report-order.pdf');  //สร้างไฟล์ PDF ชื่อว่า myReport.pdf
                        ob_end_flush();                 // ปิดการแสดงผลข้อมูลของไฟล์ HTML ณ จุดนี้
                        ?>
            <div class="input-box button text-center mb-5">
                <a href="report-order.pdf"><button class="btn btn-success">แปลงไฟล์ PDF</button> </a>
