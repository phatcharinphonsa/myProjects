...
<head>
    ...
    <title>ตารางแสดงข้อมูลการจ่ายชำระสินค้า <?php echo $name; ?></title>
...
<body>
    <section class="home-section justify-center mt-5">
        <div class="con">
            <h5 class="mt-2 mb-3">สั่งซื้อสิค้า / จ่ายชำระสินค้า
            </h5>
            <div class="mt-2 mb-3">
            ...
            <h5 class="mt-2 mb-3 text-center">ตารางแสดงข้อมูลที่ต้องจ่ายชำระสินค้า
            </h5>
            <div class="table-responsive">
                <table id="myTable" class="display" style="width: 100%;">
                    ...
                    <tbody>
                        <?php
                        $i = 1;
                        $sql = "SELECT * FROM `order` JOIN `agents` ON `order`.`agent_id` = `agents`.`agent_id` WHERE `order_status` = 1 ORDER BY reg_date DESC";
                        ...
                        ?>
                            <tr>
                                ...
                                <td>
                                    <a href='./pickup-detail.php?id=<?= $row['orderID'] ?>'>
                                        <button type="button" class='btn btn-secondary' style='margin-right: 5px;'>
                                            รายละเอียด
                                        </button></a>
                                </td>
                                <td>
                                    <button type="button" class="btn btn-danger" data-bs-toggle='modal' onclick="getOrder('<?= $row['orderID'] ?>')" data-bs-target="#insertModal">
                                        จ่ายชำระสินค้า
                                    </button>
    ...
    <div class="modal fade mt-5" id="insertModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <form method="POST" action="./changeStatusToSuccess.php" class="modal-content">
                <div class="modal-content">
                    <div class="modal-header">
                        <p class="modal-title" id="exampleModalLabel">ยืนยันการรับสินค้าหมายเลขOD<samp class="oderId"></samp></p>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body text-center">
                        <label class="mb-1 ">จ่ายชำระโดย <?php echo $name; ?> </label>
                    </div>
